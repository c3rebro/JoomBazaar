...
function execute_sql_and_send_email($conn, $sql, $email, $seller_id, $hash, $verification_token, $mailtxt) {
	...
	$verification_link = BASE_URI . "verify&token=$verification_token&hash=$hash";
...
// Function to show a message using Joomla's messaging system
function show_joomla_message($message, $type = 'info') {
    $app = JFactory::getApplication();
    $app->enqueueMessage($message, $type);
}

function show_alert_existing_request() {
    show_joomla_message('Eine Verkäufernr-Anfrage wurde bereits generiert. Pro Verkäufer ist in der Regel nur eine Verkäufernr zulässig. Bitte melden Sie sich per Mail, wenn Sie eine weitere Nummer haben möchten, oder wenn Sie Probleme haben, Ihre bereits angefragte Nummer frei zu Schalten.', 'warning');
}

function show_alert_active_id() {
    show_joomla_message('Eine Verkäufer Nummer wurde bereits für Sie aktiviert. Pro Verkäufer ist in der Regel nur eine Verkäufernr zulässig. Bitte melden Sie sich per Mail, wenn Sie eine weitere Nummer haben möchten.', 'warning');
}
