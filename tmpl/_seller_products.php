...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_product'])) {
    ...
    // Validate price
    if ($price < $min_price) {
        $validation_message = "Der eingegebene Preis ist niedriger als der Mindestpreis von $min_price €.";
    } elseif (fmod($price, $price_stepping) != 0) {
        $validation_message = "Der Preis muss in Schritten von $price_stepping € eingegeben werden.";
    } else {
...
<body>
    <div class="container">
        ...
        <?php if (isset($update_validation_message)) { ?>
            <div class="alert alert-danger"><?php echo $update_validation_message; ?></div>
        <?php } ?>
        <div class="action-buttons">
            <form action="?page=seller_products&seller_id=<?php echo $seller_id; ?>&hash=<?php echo $hash; ?>" method="post" class="w-100">
                ...
                <button type="submit" class="btn btn-primary w-100 mt-4" name="create_product">Artikel erstellen</button>
            </form>
        </div>
        <a href="<?php echo NWND_URI; ?>/print_barcodes.php?seller_id=<?php echo $seller_id; ?>&hash=<?php echo $hash; ?>" class="btn btn-secondary mt-3 w-100" target="_blank">Etiketten drucken</a>
        ...
        <!-- No Bazaar Found Modal -->
        <div class="modal fade" id="noBazaarModal" tabindex="-1" role="dialog" aria-labelledby="noBazaarModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="noBazaarModalLabel">Fehler</h5>
                    </div>
                    <div class="modal-body">
                        Artikeländerung blockiert: Keinen neuen Basar (Startdatum in der Zukunft) gefunden.
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Schließen</button>
                    </div>
    ...
    jQuery(document).ready(function($) {
        // Define the editProduct function in the global scope
        window.editProduct = function(id, name, size, price) {
            // Check if the current bazaar ID is available
            var bazaarId = <?php echo json_encode($bazaar_id); ?>;
            if (bazaarId === false) {
                $('#noBazaarModal').modal('show');
                return;
            }
        ...
        // Ensure the close button works specifically for the editProductModal
        $('#editProductModal .btn-secondary').on('click', function() {
            $('#editProductModal').modal('hide');
        });

        // Ensure the close button works specifically for the noBazaarModal
